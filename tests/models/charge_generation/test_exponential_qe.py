    ...
    qe_data = pd.DataFrame(
        {
            ...
            "reflectivity": [0.42, 0.52, 0.61, 0.69, 0.76, 0.82],
            "absorptivity": [1.5e-3, 1.8e-3, 2.1e-3, 2.4e-3, 2.7e-3, 3.0e-3],
            "c": [1.0e-4, None, 1.2e-4, 1.3e-4, 1.4e-4, None],  # NaN values in the 'c' column
        }
    )
...
def test_exponential_qe_negative(ccd_5x5: CCD, negative_qe_csv: str):
    ...
    with pytest.raises(ValueError, match="Negative values found in the following columns: absorptivity. All values for 'wavelength', 'reflectivity', and 'absorptivity' must be non-negative."):
        exponential_qe(
            detector=ccd_5x5,
...
def test_exponential_qe_missing(ccd_5x5: CCD, missing_qe_csv: str):
    ...
    with pytest.raises(ValueError, match="CSV file must contain the columns: reflectivity, absorptivity, wavelength"):
        exponential_qe(
            detector=ccd_5x5,
...
def test_exponential_qe_nan_values(ccd_5x5, nan_qe_csv):
    ...
    with pytest.raises(ValueError, match=re.escape("NaN values found in the file. All values for 'wavelength', 'reflectivity', and 'absorptivity' must be present.")):
        exponential_qe(
            detector=ccd_5x5,
            filename=nan_qe_csv,
            x_epi=0.0002,
...
def test_exponential_qe_nan_in_c_column(ccd_5x5, nan_c_column_csv):
    ...
    with pytest.raises(ValueError, match="NaN values found in the 'c' column. All values must be present."):
        exponential_qe(
            detector=ccd_5x5,
...
def test_exponential_qe_invalid_wavelength(ccd_5x5: CCD, valid_qe_csv: str):
    ...
    with pytest.raises(ValueError, match="You must specify a `default_wavelength` value in nm or use 'multi' for multiple wavelengths."):
        exponential_qe(
            detector=ccd_5x5,
...
def test_exponential_qe_invalid_wavelength_range(ccd_5x5: CCD, valid_qe_csv: str):
    ...
    with pytest.raises(ValueError, match="Wavelength is out of the valid range for the equation."):
        exponential_qe(
            detector=detector,
            filename=valid_qe_csv,
            x_epi=0.0002,
...
def test_exponential_qe_invalid_temperature(ccd_5x5: CCD, valid_qe_csv: str):
    ...
    with pytest.raises(ValueError, match="The temperature provided does not match with the environment."):
        exponential_qe(
            detector=detector,
            filename=valid_qe_csv,
            x_epi=0.0002,
...
def test_exponential_qe_invalid_detector_type(ccd_5x5: CCD, valid_qe_csv: str):
    ...
    with pytest.raises(ValueError, match=re.escape("Invalid detector type. Choose 'BI' (Back-Illuminated) or 'FI' (Front-Illuminated).")):
        exponential_qe(
            detector=detector,
            filename=valid_qe_csv,
            x_epi=0.0002,
...
def test_exponential_qe_invalid_x_epi(ccd_5x5: CCD, valid_qe_csv: str):
    ...
    with pytest.raises(ValueError, match=re.escape("x_epi cannot be greater than the total detector thickness.")):
        exponential_qe(
            detector=detector,
            filename=valid_qe_csv,
            x_epi=0.02,
...
def test_exponential_qe_random_x_epi_x_poly(ccd_5x5, valid_qe_csv):
    ...
    if x_epi < 0 or x_poly < 0:
        with pytest.raises(ValueError, match=re.escape("Epitaxial thickness (x_epi) and poly layer thickness (x_poly) must be non-negative.")):
            exponential_qe(
                detector=ccd_5x5,
                filename=valid_qe_csv,
                x_epi=x_epi,
...
def test_photon_array_2d_with_multi(ccd_5x5, valid_qe_csv):
    ...
    with pytest.raises(ValueError, match=re.escape("Photon array is 2D, but you specified 'multi' for `default_wavelength`.")):
        exponential_qe(
            detector=ccd_5x5,
            filename=valid_qe_csv,
            x_epi=0.0002,
