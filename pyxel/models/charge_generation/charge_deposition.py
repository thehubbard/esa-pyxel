...
def simulate_charge_deposition(
    ...
    if energy_spectrum is None:
        ...
        if energy_spectrum_sampling == "log":
            log_energy_range = np.logspace(np.log10(np.min(energy_spectrum_data[:, 0])),
            ...
            lin_energy_range = np.linspace(np.min(energy_spectrum_data[:, 0]), np.max(energy_spectrum_data[:, 0]),
                                           n_samples)
            energy_pdf = np.interp(lin_energy_range, energy_spectrum_data[:, 0], energy_spectrum_data[:, 1])
            energy_pdf /= np.sum(energy_pdf)
            p_energies = np.random.choice(lin_energy_range, n_p, p=energy_pdf)
    p_energies *= 1.e6  # from MeV to eV

    # extract stopping power data convert to correct units and compute initial deposited energy
    stopping_power_data = np.genfromtxt(stopping_power_curve, skip_header=1, delimiter=',')
    stopping_power_data[:, 0] *= 1.e6  # from MeV to eV
    stopping_power_data[:, 1] *= material_density * 1.e2 * step_size  # from MeV cm2/g to eV
    deposited_energies = np.interp(p_energies, stopping_power_data[:, 0], stopping_power_data[:, 1])
    ...
    for e, de in zip(p_energies, deposited_energies):
        ...
        if (e - n_steps * de) < 0:
            ...
            for step in range(n_steps):
                vde[step] = np.interp(e, stopping_power_data[:, 0], stopping_power_data[:, 1])
                e -= de
