...
def simple_persistence(
    ...
    if 'persistence' not in detector._memory.keys():
        ...
        for trap_density, trap_timeconstant in zip(trap_densities, trap_timeconstants):
            entry = ''.join(['trappedCharges_',
                             str(trap_density),
                             '-',
                             str(trap_timeconstant)])
            detector._memory['persistence'].update({entry:np.zeros((detector.geometry.row, detector.geometry.col))})
    else:
        for trap_density, trap_timeconstant in zip(trap_densities, trap_timeconstants):
            entry = ''.join(['trappedCharges_',
                             str(trap_density),
            ...
            trapped_charges = trapped_charges + (0.5 / trap_timeconstant) * (detector.pixel.array * trap_density - trapped_charges) 
